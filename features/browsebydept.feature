Feature: Browse by department/all 
	...
	Scenario: subdepartments should be listed
		...
		Languages, Literatures & Cultures should be listed as 
		headers with their classes listed below them
	...
	Scenario: professors who have taught the course should be 
		...
		Then the professors should be listed in alphabetical 
		order by last name 
		Then the number of reviews for each professor should 
		be listed to the right of the corresponding professor
		Then the ratings for each professor should be listed 
		to the right of the corresponding professor 
	...
	Scenario: professor is clicked from a list of professors
		...
		Then CHIN 1020 - Elementary Chinese should appear at 
		the top
		Then a menu to select different professors that teach 
		the CHIN 1020 - Elementary Chinese should appear
	...
	Scenario: professor is clicked from a list of professors
		...
		Then a menu to select different professors that teach 
		the PSYC 2210 Animal Behavior should appear
		Then "Write a Review" button should appear
