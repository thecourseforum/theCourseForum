Feature: Write a tCF Review
	...
	Scenario: page should be found
		...
		Then I should see 'Write a Review', fields for department, course, professor, grades 
		And a text field

	Scenario: should be able to select a valid department
		Given the link'http://localhost:3000'
		When I click on Write a Review
		Then I should be able to click on the "Department" dropdown and see every valid department
